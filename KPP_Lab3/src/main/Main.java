...
public class Main extends Application {
    ...
    public void start(final Stage primaryStage) {
        ...
        directorVBox1.getChildren().addAll(addProblem, rewardCoder,
                directorCashHBox);

        Label directorLabel = new Label("Director " + "["
                + String.valueOf(Company.DEFAULT_DIRECTOR_SALARY) + "]");
        ...
        solvedProblemsHBox.getChildren().addAll(solvedProblems,
                solvedProblemsValue);
        VBox coderVBox1 = new VBox();
        coderVBox1.setSpacing(10);
        coderVBox1.getChildren().addAll(solveProblem, askForRaise,
                coderCashHBox, solvedProblemsHBox);

        Label coderLabel = new Label("Coder " + "["
                + String.valueOf(Company.DEFAULT_CODER_SALARY) + "]");
        ...
        Label cleanerLabel = new Label("Cleaner " + "["
                + String.valueOf(Company.DEFAULT_CLEANER_SALARY) + "]");
        ...
        solveProblem.setOnAction(event -> {
            ...
            solvedProblemsValue.setText(
                    String.valueOf(company.getCoderSolvedProblems()));
            problemCountInt.setText(company.problemCount());
        });
        ...
        paySalary.setOnAction(event -> {
            ...
            directorCashValue.setText(
                    String.valueOf(oldDirectorCash
                            + company.getDirectorSalary()));
            int oldCoderCash = Integer.parseInt(coderCashValue.getText());
            coderCashValue.setText(
                    String.valueOf(oldCoderCash + company.getCoderSalary()));
            int oldCleanerCash = Integer.parseInt(cleanerCashValue.getText());
            cleanerCashValue.setText(
                    String.valueOf(oldCleanerCash
                            + company.getCleanerSalary()));
        });

        rewardCoder.setOnAction(event -> {
            int oldCoderCash = Integer.parseInt(coderCashValue.getText());
            coderCashValue.setText(String.valueOf(oldCoderCash
                    + Company.DEFAULT_AWARD));
        });
